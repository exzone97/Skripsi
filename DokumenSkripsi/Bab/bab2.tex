...
\subparagraph{Energi Efisien} setiap node sensor memiliki keterbatasan energi atau daya yang dimiliki. Membuat protokol untuk menghemat penggunaan energi adalah yang dibutuhkan dalam WSN.
...
%11.4.3.1 hal 397 book wireless sensor network - a network perspective
%\subsubsection{Sensor Transmission Control Protocol (STCP)}
%STCP adalah protokol transport End-to-End upstream transport untuk WSN yang mendukung kontrol kemacetan jaringan (congestion control) dan reliability. Pada STCP, node sensor menggunakan RED (Random Early Detection) yaitu algoritma untuk mendeteksi kemacetan berdasarkan panjang antrian. Jika terjadi kemacetan jaringan node sensor akan menambahkan 1 bit pada paket yang akan dikirim tersebut. Sink node akan menerima 1 bit tersebut dan memberitahu node sensor untuk mengirimkan paket data dengan menggunakan ACK atau NACK. Node sensor dapat memilih rute lain atau mengurangi jumlah data yang dikirim untuk mengurangi kemacetan jaringan. 

%Salah satu yang ditawarkan oleh STCP adalah kontrol keandalan (reliability) melalui End-to-End Retransmission. Dengan kata lain STCP menggabungkan berbagai macam fungsi transfer dengan protokol transfer. Pada STCP terdapat proses awal dimana node sensor menginformasikan sink node fitur aplikasinya seperti jenis aliran data, tingkat transmisi, dan reliabilitas yang dibutuhkan.

%\subsubsection{Price-Oriented Reliable Transport Protocol (PORT)}


%11.4.2.4 hal 396 book wireless sensor network - a network perspective
%\subsubsection{GARUDA}
%GARUDA adalah protokol yang memastikan reliabilitas downstream pada WSN. GARUDA membuat dua tier arsitektur untuk pengiriman ulang (retransmission) data. 

%\subsubsection{Delay Sensitive Transport (DST)}

%11.4.2.3 hal 395 book wireless sensor network - a network perspective
%\subsubsection{Pump Slowly, Fetch Quickly (PSFQ)}
%Pump Slowly, Fetch Quickly (PSFQ) adalah protokol transport pada downstream pada WSN yang memiliki tujuan untuk mendistribusikan data dari sink node secara reliable. Cara yang dilakukan oleh PSFQ adalah dengan meminimalkan biaya untuk pemulihan data yang hilang 

%11.4.3.2 hal 397 book wireless sensor network - a network perspective
%\subsubsection{Event-to-Sink Reliable Transport (ESRT)}
%Event-to-Sink Reliable Transport adalah protokol transport End-to-End upstream yang mendukung reliabilitas. Pada ESRT tingkat pengiriman pada setiap node sensor bergantung pada tingkat reliabilitas di sink node begitu juga dengan status jaringan tersebut (terdapat kemacetan atau tidak). ESRT adalah protokol pertama yang membahas kontrol kongesti (congestion control) dan keandalan (reliability) sekaligus. Pada ESRT setiap node sensor mendeteksi kongesti berdasarkan peningkatan buffer lalu mengatur CN bit pada header sebuah paket dan meneruskannya ke sink node. Berdasarkan paket yang diterima oleh sink node, sink node dapat mengetahui keadaan dari jaringan dan juga keandalan paket untuk menentukan jumlah paket yang telah berhasil diterima pada periode tertentu. 

%ESRT bergantung pada tingkat pengaturan pada node sensor untuk memastikan data yang reliable. Terdapat dua masalah yang dapat terjadi yaitu: energi yang tidak efisien jika dibandingkan dengan retransmission dan tidak dapat menjamin reliabilitas jika terjadi kongesti. Namun, untuk meningkatkan reliabilitas dapat dilakukan recovery loss (pemulihan dari data yang hilang). ESRT masih membutuhkan jalur yang khusus dengan daya yang besar untuk mengirimkan notifikasi kepada node sensor yang akan mempengaruhi proses pengiriman data. 

%11.4.1.2 hal 391 book wireless sensor network - a network perspective
%\subsubsection{Congestion Detection and Avoidance (CODA)}
%CODA adalah protokol untuk mengatur kemacetan (congestion control). CODA akan mendeteksi kemacetan berdasarkan buffer dan media jaringan wireless tersebut. CODA menggunakan penekanan pesan untuk memberitahu apakah terdapat kemacetan kepada node sensor. Saat node mendapatkan pesan tersebut, node akan memperkecil tingkat pengiriman. Dilain sisi node tetangganya akan meningkatkan tingkat pengiriman jika tidak menerima pesan tersebut. Pesan dan ACK pada CODA akan menggunakan lebih banyak daya.
